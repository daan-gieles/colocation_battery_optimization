...
def generate_output(starting_soc=0.1, p_limit=0.1, date='2025-01-15', charge_from_grid=True, etp_cost=5, average_bid_ask_spread=1):
    """
    Runs the optimization, plots results
    """

    model_colocation, _ = run_single_optimization_colocation(
    ...
    model_battery, _ = run_single_optimization_colocation(starting_soc=starting_soc, p_limit=p_limit, date=date,
    ...
    ax1.set_title(
        f'Single-shot optimization on {date}. Max power {str(round(p_limit, 2))}, starting SOC {str(round(starting_soc, 1))}', fontsize=9)
