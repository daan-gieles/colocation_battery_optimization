...
def generate_output(starting_soc = 0.1, p_limit = 0.1,date='2025-01-15',charge_from_grid=True,etp_cost = 5):
    model_colocation,_ = run_single_optimization_colocation(starting_soc = starting_soc, p_limit = p_limit,date=date,charge_from_grid=charge_from_grid,etp_cost=etp_cost)
    model_battery,_ = run_single_optimization_colocation(starting_soc = starting_soc, p_limit = p_limit,date=date,charge_from_grid=charge_from_grid,no_pv=True,etp_cost=etp_cost)
    ...
    ax1.set_ylabel('SOC / PV / Battery Power',fontsize=9)
    ax2.set_ylabel('DA prices',fontsize=9)
    ax3.set_ylabel('P&L',fontsize=9)
    ...
    # plt.show()
    ax3.tick_params(rotation=45,labelsize=9,axis='x')
    ax1.legend()
    ax2.legend()
    ax3.legend()
...
def run_single_optimization_colocation(starting_soc = 0.1, p_limit = 0.1,date='2025-01-15',charge_from_grid=True,no_pv=False,etp_cost=5):
    ...
    def objective_expression(model):
        ...
        return sum(-model.da[t]*(model.p[t]-model.pv[t]) for t in model.t)-(sum(model.p[t]**2 for t in model.t)*etp_cost/4)**(0.5)
