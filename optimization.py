...
def generate_output(starting_soc = 0.1, p_limit = 0.1,date='2025-01-15',charge_from_grid=True,etp_cost = 5,average_bid_ask_spread = 1):
    
    model_colocation,_ = run_single_optimization_colocation(starting_soc = starting_soc, p_limit = p_limit,date=date,charge_from_grid=charge_from_grid,etp_cost=etp_cost,average_bid_ask_spread=average_bid_ask_spread)
    model_battery,_ = run_single_optimization_colocation(starting_soc = starting_soc, p_limit = p_limit,date=date,charge_from_grid=charge_from_grid,no_pv=True,etp_cost=etp_cost,average_bid_ask_spread=average_bid_ask_spread)
    ...
    prevented_slippage = calculate_kpis.calculate_prevented_slippage(model_colocation,average_bid_ask_spread=average_bid_ask_spread)
    ...
    ax1.plot(ts_series,pv_to_battery_series,label='PV to battery [MW]',linestyle='--')
    
    ax2.plot(ts_series,accumulated_pnl_series_colocation,label=f'Colocation: {round(accumulated_pnl_series_colocation[-1],2)}EU')
    ax2.plot(ts_series,accumulated_pnl_series_pv,label=f'PV Only: {round(accumulated_pnl_series_pv[-1],2)}EU')
    ax2.plot(ts_series,accumulated_pnl_series_battery_only,label=f'Battery Only: {round(accumulated_pnl_series_battery_only[-1],2)}EU')
    ...
    ax1.set_title(f'Single-shot optimization on {date}. Max power {str(round(p_limit,2))}, starting SOC {str(round(starting_soc,1))}',fontsize=9)
...
def run_single_optimization_colocation(starting_soc = 0.1, p_limit = 0.1,date='2025-01-15',charge_from_grid=True,no_pv=False,etp_cost=5,average_bid_ask_spread = 1):
